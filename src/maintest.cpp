...
void init_sensors_after_powerup() {
...
void app_setup_test() {
  ...
  Serial.println("Sensor power held HIGH in setup; will power ON in loop.");
}

void app_loop_test() {
  if (!g_sensors_powered) {
    // Sensor power rail is active-low on this board.
    digitalWrite(kPinSensorPower, LOW);
    delay(kSensorPowerWarmupMs);
    g_sensors_powered = true;
    Serial.println("Sensor power ON (GPIO47 LOW), warm-up done.");
  }
  if (!g_sensors_initialized) {
    init_sensors_after_powerup();
    read_measurements();
